...
`watch_interval` seconds (0 = read once), or the list itself. Every key but `devices` has a default. The bridge's own
topic templates are read from its retained `{root}/bridge/config` at start.
"""
...
class Config:
    ...
    def from_dict(cls, data: dict, base: Path | None = None) -> "Config":
        unknown = set(data) - {"bridge", "il", "devices", "options", "watch_interval"}
        if unknown:
